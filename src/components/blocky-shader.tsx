...
function BlockyShader({ paused = false, className, ...props }: BlockyShaderProps) {
  ...
  useEffect(() => {
    ...
    let fg = isDark() ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.07)"

    const updateColors = () => {
      fg = isDark() ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.07)"
    }
    ...
    const render = (now: number) => {
      ...
      const t = ((now - startTime) / 1000) * 0.3
      const t3 = t / 3
      const aspect = w / h
