...
function FieldLabel({ className, ...props }: React.ComponentProps<typeof Label>) {
  return (
    <Label
      ...
      className={cn(
        "group/field-label peer/field-label flex w-fit gap-2 font-semibold leading-snug has-[>[data-slot=field]]:rounded-md has-[>[data-slot=field]]:border has-data-checked:border-primary/30 has-data-checked:bg-primary/5 *:data-[slot=field]:p-3 group-data-[disabled=true]/field:opacity-50 dark:has-data-checked:border-primary/20 dark:has-data-checked:bg-primary/10",
...
function normalizeError(error: FieldErrorItem): string | undefined {
...
function FieldError({
  ...
  const content = useMemo(() => {
    ...
    const messages = errors.map(normalizeError).filter(Boolean) as string[]
