  ...
  return (
    ...
    >
      ...
      <div
        ...
        data-side={side}
        className={cn(
          "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear data-[side=right]:right-0 data-[side=left]:left-[var(--sidebar-left-offset,0px)] data-[side=right]:group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)] data-[side=left]:group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-left-offset,0px)-var(--sidebar-width))] md:flex",
...
function SidebarMenuButton({
  ...
  // When tooltip is present, wrap in Tooltip but preserve the render prop
  // so the element renders as the correct tag (e.g. <a> via Next.js Link)
  const triggerRender = render ?? <button />
  return (
    <Tooltip>
      <TooltipTrigger
