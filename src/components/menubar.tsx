...
function Menubar({ className, ...props }: MenubarPrimitive.Props) {
  return (
    <MenubarPrimitive
      data-slot="menubar"
      className={cn(
        "flex h-9 items-center gap-1 rounded-md border bg-card p-1 shadow-xs",
        className,
      )}
      {...props}
    />
...
function MenubarSubContent({
  ...
  return (
    <DropdownMenuSubContent
      ...
      className={cn(
        "data-closed:fade-out-0 data-open:fade-in-0 data-closed:zoom-out-95 data-open:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 min-w-32 rounded-lg border border-border-light bg-popover p-1 text-popover-foreground shadow-[var(--shadow-lifted)] duration-100 data-closed:animate-out data-open:animate-in",
        className,
      )}
      {...props}
    />
